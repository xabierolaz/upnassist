...
function elToMd(el) {
    ...
    // Sample Output
    if (el.classList && (el.classList.contains('SampleOutput__Wrapper-sc-8wht92-0') || 
        (el.tagName === 'DIV' && el.querySelector && el.querySelector('.SampleOutput__Note-sc-8wht92-2')))) {
         return "```text\n" + el.textContent.replace("Sample output", "").trim() + "\n```\n\n";
    }

    // Fallback for leaf text
...
function parseHtml(htmlContent, sectionTitle) {
    ...
    // Recursive processor that pushes to contentMap
    function process(el) {
        if (!el) return;
        
        // Skip
        if (['SCRIPT', 'STYLE', 'SVG', 'NOSCRIPT'].includes(el.tagName)) return;
        if (el.classList && (el.classList.contains('EndOfSubSection__ButtonWrapper-ees48a-2') || el.classList.contains('CoursePageFooter__CoursePageFooterWrapper-sc-1wp274x-0'))) return 'STOP';
        if (el.textContent && el.textContent.includes("You have reached the end of this section")) return 'STOP';
        ...
        // Check if it is a "Leaf" block we handle directly
        const isCode = el.tagName === 'PRE' || (el.tagName === 'DIV' && el.classList.contains('gatsby-highlight'));
        const isSample = el.classList && (el.classList.contains('SampleOutput__Wrapper-sc-8wht92-0') || (el.tagName === 'DIV' && el.querySelector && el.querySelector('.SampleOutput__Note-sc-8wht92-2')));
        const isList = el.tagName === 'UL' || el.tagName === 'OL';
        const isHeader = /^H[1-6]$/.test(el.tagName);
        const isPara = el.tagName === 'P';

        if (isCode || isSample || isList || isHeader || isPara) {
            currentText += elToMd(el);
            return;
        }

        // Wrapper Divs -> Recurse
        if (['DIV', 'ASIDE', 'SECTION', 'ARTICLE', 'MAIN', 'BLOCKQUOTE'].includes(el.tagName)) {
            for (const child of Array.from(el.children)) {
                if (process(child) === 'STOP') return 'STOP';
            }
        } else {
            // Unknown leaf
            currentText += elToMd(el);
    ...
    for (let i = startIndex; i < children.length; i++) {
        if (process(children[i]) === 'STOP') break;
    }
    flush();
