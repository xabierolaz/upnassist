...
class TestChange(unittest.TestCase):
    def test_run(self):
        ...
        # [1, 2, 10, 4, 5]
        if "[1, 2, 10, 4, 5]" not in out:\n             self.fail("La lista no se ha modificado correctamente. En el índice 2 debería aparecer un 10.")
...
class TestTwice(unittest.TestCase):
    def test_run(self):
        ...
        # 3 different words
        if "3" not in out:
             self.fail("Debe imprimir 3 (one, two, three). Recuerda usar 'if palabra in lista' para detectar duplicados.")
        self.assertIn("3", out)
`
    },
    {
