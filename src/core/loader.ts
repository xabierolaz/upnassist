...
/** Error thrown when loading a unit fails */
export class UnitLoadError extends Error {
    constructor(
        ...
        super(`Failed to load unit "${unitId}" from course "${courseId}"`);
 ...
 * @throws {UnitLoadError} When loading fails
 */
export const loadUnit = async (courseId: string, unitId: string): Promise<CoursePage | undefined> => {
    try {
        ...
        throw new UnitLoadError(courseId, unitId, error);
    }
};
